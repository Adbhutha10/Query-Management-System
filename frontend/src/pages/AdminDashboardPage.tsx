...
import { LogOut, CheckCircle, LayoutGrid, Download } from 'lucide-react';
...
const AdminDashboardPage: React.FC = () => {
    ...
    const fetchQueries = async () => {
        if (!token) return;
        try {
            const response = await fetch(`${API_URL}/api/admin/queries`, {
    ...
    const handleOpenReplyModal = (queryId: string) => {
    ...
    const handleReplySubmit = async () => {
        if (!token || !currentQueryId || !replyText.trim()) return;
        try {
    ...
    const handleExportExcel = async () => {
        if (!token) return;
        try {
            const response = await fetch(`${API_URL}/api/admin/download/excel`, {
                headers: { 'X-ADMIN-ID': token }
            });
            ...
            a.href = url;
            a.download = `queries_report_${new Date().toLocaleDateString()}.xlsx`;
        ...
        <div className="flex flex-col md:flex-row min-h-screen bg-secondary-light font-body text-text-main">
                    ...
                    <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 mt-2 gap-4">
                        ...
                        <div className="flex items-center gap-3 self-start md:self-auto">
                            <button
                                ...
                                className="flex items-center gap-2 bg-white/80 backdrop-blur-md px-5 py-2 rounded-full text-sm font-bold text-primary shadow-sm border border-primary/20 hover:bg-white hover:-translate-y-0.5 transition-all duration-200"
                            >
                                <Download size={16} /> Download Report
                            </button>
                            <span className="bg-white/80 backdrop-blur-md px-5 py-2 rounded-full text-sm font-bold text-text-main shadow-sm border border-white/60">
                                    ...
                                    {currentQueries.length > 0 ? (
                                        currentQueries.map((q) => (
                                            <tr key={q.queryId} className="hover:bg-white/60 transition-colors border-b border-slate-100 last:border-0 group">
                                                ...
                                                <td className="py-3 px-5">
                                                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border ${q.priority === 'HIGH' ? 'bg-red-50 text-red-700 border-red-200' :
                                                        q.priority === 'MEDIUM' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                                                ...
                                                <td className="py-3 px-5">
                                                    {q.status !== 'RESOLVED' && (
                                                        <button
                                                            ...
                                                            onClick={() => handleOpenReplyModal(q.queryId)}
                                                        >
                                        ...
                                        <tr>
                                            <td colSpan={6} className="p-8 text-center text-text-muted italic">No queries found.</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
            ...
            {isReplyModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-md animate-fade-in">
                    <div className="bg-white/90 backdrop-blur-2xl w-full max-w-lg rounded-3xl shadow-2xl border border-white/50 overflow-hidden transform animate-scale-in">
                        <div className="p-8">
                            <h3 className="font-heading text-2xl font-bold text-text-main mb-2">Send Custom Reply</h3>
                            <p className="text-text-muted text-sm mb-6 font-medium">This message will be sent to the user's email and the query will be marked as resolved.</p>

                            <textarea
                                className="w-full h-40 p-4 bg-white/50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all resize-none text-text-main font-medium placeholder:text-text-muted/50"
                            ...
                            <div className="flex gap-4 mt-8">
                                <button
                                    className="flex-1 p-4 rounded-2xl border border-slate-200 bg-white text-text-muted font-bold hover:bg-slate-50 transition-all hover:-translate-y-0.5"
                                    onClick={() => setIsReplyModalOpen(false)}
                                >
                                    Cancel
                                </button>
                                <button
                                    className="flex-1 p-4 rounded-2xl bg-gradient-to-br from-primary to-primary-dark text-white font-bold shadow-lg shadow-primary/20 hover:-translate-y-0.5 transition-all disabled:opacity-50 disabled:transform-none"
