...
const DashboardPage: React.FC = () => {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const qRes = await fetch(`${API_URL}/api/queries/user/${userId}`, {
    ...
    const handleQuerySubmit = async (e: React.FormEvent) => {
        ...
        setMessage('');
        try {
            const response = await fetch(`${API_URL}/api/queries/submit`, {
                method: 'POST',
                headers: {
                                ...
                                <table className="w-full min-w-[600px] border-collapse text-left">
                                    <thead>
                                        <tr>
                                            ...
                                            <th className="bg-secondary-light/50 p-5 font-heading font-bold text-sm tracking-wide text-primary-dark uppercase">Admin Response</th>
                                    ...
                                    <tbody>
                                        {queries.length > 0 ? queries.map((q) => (
                                            <tr key={q.queryId} className="hover:bg-white/60 transition-colors border-b border-slate-100 last:border-0">
                                                ...
                                                <td className="p-5 font-medium text-text-main max-w-[250px] truncate">{q.originalQuery}</td>
                                                <td className="p-5">
                                                    {q.adminReply ? (
                                                        <div className="bg-emerald-50/50 border border-emerald-100 p-3 rounded-xl text-sm text-emerald-800 font-medium leading-relaxed">
                                                            <span className="block text-[10px] uppercase font-bold text-emerald-600 mb-1">Official Reply:</span>
                                                            {q.adminReply}
                                                        </div>
                                                    ) : (
                                                        <span className="text-text-muted italic text-sm">Awaiting response...</span>
                                                    )}
                                                </td>
                                                <td className="p-5">
                                            ...
                                            <tr>
                                                <td colSpan={5} className="text-center p-16 text-text-muted text-lg font-medium">
